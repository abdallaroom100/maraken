...
const EditInvoice: React.FC = () => {
  ...
  const [clientType, setClientType] = useState<'فردي' | 'تجاري' | ''>('');
  ...
  const fetchInvoice = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setProducts(invoice.products.map((p: Product) => ({
          ...p,
          total: calculateProductTotal(p)
        })));

        if (invoice.clientType === 'فردي') {
          setIndividualClient(invoice.clientData);
        } else {
          setBusinessClient(invoice.clientData);
        }
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/invoice/update/${id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        toast.error(error.error || 'حدث خطأ في تحديث الفاتورة');
      }
    } catch (error) {
